...
const STEP_INFO: StepInfo[] = [
  {
    title: "三条执行通道",
    description:
      "Agent 除了主线程，还可以拉起守护型后台线程并行处理任务。",
  },
  {
    title: "主线程工作中",
    description:
      "主 Agent 循环照常运行，持续处理用户请求。",
  },
  {
    title: "启动后台任务",
    description:
      "后台任务以守护线程形式运行，主循环不会等待它们结束。",
  },
  {
    title: "多个后台并发",
    description: "多个后台任务可以同时运行。",
  },
  {
    title: "任务完成",
    description:
  ...
  },
  {
    title: "队列累积",
    description:
      "结果会先堆积在队列里，在这一轮推理期间模型还看不到它们。",
  },
  {
    title: "清空队列",
    description:
      "在下一次 LLM 调用前，所有排队通知会统一注入为 tool_result，整个过程是非阻塞异步的。",
  },
];
        ...
        <svg viewBox="0 0 780 380" className="w-full" aria-label="Background task lanes">
          ...
          {(
            [
              ...
              { key: "bg1", y: LANE_Y.bg1, label: "后台线程 1" },
              { key: "bg2", y: LANE_Y.bg2, label: "后台线程 2" },
            ] as const
          ).map(({ key, y, label }) => (
            <g key={key}>
              <rect
